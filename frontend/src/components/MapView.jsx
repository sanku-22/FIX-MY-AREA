...
function makeIcon(issue) {
  ...
  const svg = ICON_SVGS[cat.icon] || ICON_SVGS.CircleHelp;
  const html = `
    <div class="cf-marker" style="background:${cat.color};border-color:${status.color}">
      <svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round">${svg}</svg>
    </div>`;
  return L.divIcon({
    html,
    ...
    >
      ...
      {draggableMarker && (
        <Marker
          ...
          draggable={true}
          icon={L.divIcon({
            html: '<div class="cf-marker" style="background:#09090b;border-color:#fff"><svg viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 6-8 12-8 12s-8-6-8-12a8 8 0 0 1 16 0z"/><circle cx="12" cy="10" r="3"/></svg></div>',
            className: "cf-div-icon",
            iconSize: [40, 40],
            iconAnchor: [20, 40],
          })}
